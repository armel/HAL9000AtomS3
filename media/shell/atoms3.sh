...
for filename in *; do
    ...
    # Scale
    ffmpeg -i "$filename" -ss 8 -vf scale=-1:128 -vcodec libx265 -crf 24 -an -y /tmp/tmp.mp4
    # Crop
    ffmpeg -i /tmp/tmp.mp4 -filter:v "crop=128:128:49:0" -c:a copy -an -y /tmp/crop.mp4
    # Grab miniature jpg
